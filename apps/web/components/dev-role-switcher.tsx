...
export function DevRoleSwitcher() {
    ...
    useEffect(() => {
        ...
        return () =>
            window.removeEventListener(DEV_USER_CHANGED_EVENT, onChange);
    }, []);
    ...
    const sessionUser = sessionData?.user
        ? {
              ...
              name:
                  (sessionData.user as { name?: string | null }).name ??
                  sessionData.user.email ??
                  "Session user",
              email: sessionData.user.email,
              role: ((sessionData.user as { role?: DevUserRole }).role ??
                  "INVESTOR") as DevUserRole,
          }
        : null;
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger
                className={cn(
                    "group inline-flex h-10 items-center gap-2.5 rounded-full border border-emerald-200/80 bg-background pr-3 pl-1 text-left text-xs font-medium shadow-sm transition",
            ...
            >
                ...
                <span className="flex min-w-0 flex-col leading-tight">
                    ...
                    <span className="truncate text-sm font-semibold text-emerald-950 dark:text-emerald-400">
                        {active
                            ? active.label
                            : sessionIsActive
                                ...
                                >
                                    <span
                                        className={cn(
                                            ...
                                            ROLE_META[sessionUser.role]
                                                ?.iconWrap ??
                                                ROLE_META.INVESTOR.iconWrap,
                                        )}
                                    >
                                        {(() => {
                                            const Icon =
                                    ...
                                    <span
                                        className={cn(
                                            ...
                                            ROLE_META[sessionUser.role]?.chip ??
                                                ROLE_META.INVESTOR.chip,
                                        )}
                                    >
